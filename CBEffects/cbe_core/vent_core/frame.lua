...
function lib_ventframe.newFrame(params, preset)
	...
	local vent = {
		...
		lifeAlpha          = fnn(params.lifeAlpha, params.alpha, preset.lifeAlpha, 1),
		endAlpha           = fnn(params.endAlpha, preset.endAlpha, 0),
		inTime             = fnn(params.inTime, params.fadeInTime, preset.inTime, 0),
		lifeTime           = fnn(params.lifeTime, params.lifeStart, preset.lifeTime, 0),
		outTime            = fnn(params.outTime, params.lifeSpan, preset.outTime, 1000),
		...
		cyclePoint         = fnn(params.cyclePoint, params.iteratePoint, preset.cyclePoint, false),
		...
		radius             = fnn(params.radius, params.posRadius, preset.radius, 30),
		xRadius            = fnn(params.xRadius, params.radius or params.posRadius or preset.radius, preset.xRadius, 30),
		yRadius            = fnn(params.yRadius, params.radius or params.posRadius or preset.radius, preset.radius, 30),
		innerRadius        = fnn(params.innerRadius, params.posInner, preset.innerRadius, 1),
		innerXRadius       = fnn(params.innerXRadius, params.innerRadius or params.posInner or preset.innerRadius, preset.innerXRadius, 1),
		innerYRadius       = fnn(params.innerYRadius, params.innerRadius or params.posInner or preset.innerRadius, preset.innerYRadius, 1),
		...
		scaleRateX         = fnn(params.physics.scaleRateX, params.physics.sizeX, preset.physics.scaleRateX, 1),
		scaleRateY         = fnn(params.physics.scaleRateY, params.physics.sizeY, preset.physics.scaleRateY, 1),
		maxScaleX          = fnn(params.physics.maxScaleX, params.physics.maxX, preset.physics.maxScaleX, 3),
		maxScaleY          = fnn(params.physics.maxScaleY, params.physics.maxY, preset.physics.maxScaleY, 3),
		minScaleX          = fnn(params.physics.minScaleX, params.physics.minX, preset.physics.minScaleX, 0.01),
		minScaleY          = fnn(params.physics.minScaleY, params.physics.minY, preset.physics.minScaleY, 0.01),
		velFunction        = fnn(params.physics.velFunction, preset.physics.velFunction, function() end),
		useVelFunction     = fnn(params.physics.useVelFunction, params.physics.useFunction, preset.physics.useVelFunction, false),
		divisionDamping    = fnn(params.physics.divisionDamping, preset.physics.divisionDamping, true),
		autoAngle          = fnn(params.physics.autoAngle, preset.physics.autoAngle, true),
		angles             = fnn(params.physics.angles, preset.physics.angles, {{1, 360}}),
		preCalculateAngles = fnn(params.physics.preCalculateAngles, params.physics.preCalculate, preset.physics.preCalculateAngles, true),
		cycleAngle         = fnn(params.physics.cycleAngle, params.physics.iterateAngle, preset.physics.cycleAngle, false),
